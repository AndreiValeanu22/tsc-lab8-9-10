...
def test_add_and_list_tasks(regtest):
    ...
    """
    manager = TaskManager()
    manager.add_task("Primul task")
    manager.add_task("Al doilea task")
    regtest.write(manager.list_tasks())
...
def test_delete_task(regtest):
    ...
    """
    manager = TaskManager()
    manager.add_task("Task sters")
    manager.add_task("Task ramas")
...
def test_edit_task(regtest):
    ...
    """
    manager = TaskManager()
    manager.add_task("Descriere veche")
    manager.edit_task(0, "Descriere noua")
    regtest.write(manager.list_tasks())
...
def test_combination_operations(regtest):
    ...
    """
    manager = TaskManager()
    manager.add_task("Primul")
    manager.add_task("Al doilea")
    manager.mark_done(1)
    manager.edit_task(0, "Primul editat")
...
def test_invalid_index_edit(regtest):
    ...
    manager.add_task("Singurul")
    try:
        manager.edit_task(2, "Nu se aplică")
    except IndexError as exc:
        regtest.write(str(exc))
